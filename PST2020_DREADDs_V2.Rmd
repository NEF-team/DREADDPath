...
data4R <- read.delim(paste0(path,"data4R_080124.txt")) # carefull to insert the header in the txt file: trial_nb	singe	session	choice	condition
...
HOMER_99$portes <- factor(HOMER_99$portes, levels = c("clear", "blue"))
DONUT_99$portes <- factor(DONUT_99$portes, levels = c("clear", "blue"))
...
g3 <- ggplot(subset(agg.data4B, portes=='blue'), aes(x=as.factor(Injection), y=trial   , fill=Injection))+
    geom_boxplot()+
  geom_jitter(aes(color=Injection), width=0.1) +
    facet_grid(singe~portes)+
    scale_fill_manual(values=c("grey","chartreuse3","dodgerblue1"))+
...
ho.trial.glm <- glm(trial ~ Injection , family ="poisson", data=subset(agg.data4B,singe=="Homer"))
...
#- blue only
ho.brial.glm <- glm(trial ~ Injection , family ="poisson", data=subset(agg.data4B,singe=="Homer" & portes=="blue"))
summary(ho.brial.glm)

#-- DONUT

do.trial.glm <- glm(trial ~ Injection , family ="poisson", data=subset(agg.data4B,singe=="Donut"))
...
#- blue only
do.brial.glm <- glm(trial ~ Injection , family ="poisson", data=subset(agg.data4B,singe=="Donut" & portes=="blue"))
...
But Statistics on Opaque (blue) doors only shows an increase of total number of trials in DCZ sessions compared to Sham for the two monkeys.
...
ggplot(subset(agg.maxcor, portes=='blue'), aes(x=Injection,y=trial_nb, fill=Injection))+
  geom_boxplot()+
   geom_jitter(aes(colour=Injection), size=2,width = 0.1)+
    facet_grid(.~portes*singe)+
    scale_fill_manual(values=c("grey","chartreuse3","dodgerblue1"))+
...
ho.maxcor.glm <- glm(trial_nb ~ Injection , family ="poisson", data=subset(agg.maxcor,singe=="Homer" & portes=="blue"))
summary(ho.maxcor.glm)
...
do.maxcor.glm <- glm(trial_nb ~ Injection , family ="poisson", data=subset(agg.maxcor,singe=="Donut" & portes=="blue"))
summary(do.maxcor.glm)

```

Both monkeys show a longer time to get to their last reward in session especially in Blue condition (tendency 0.07) for Homer.
...
ggplot(subset(agg.NBcor, portes=='blue'), aes(x=Injection, y=trial, fill=Injection))+
  geom_boxplot()+
   geom_jitter(aes(colour=Injection), size=2,width = 0.1)+
    facet_grid(.~portes*singe)+
    scale_fill_manual(values=c("grey","chartreuse3","dodgerblue1"))+
...
short.pause <- subset(Pause4R_99, choice==99 & Injection !="no" & Injection !="0")
...
hom_pause <- glm(choice ~ Injection * portes, family="poisson",data = subset(shortp, singe=="Homer"))
summary(hom_pause)

hom_pause <- glm(choice ~ Injection * portes, family="poisson",data = subset(shortp, singe=="Donut"))
summary(hom_pause)
```
...
```{r counts, echo=FALSE,message=FALSE, warning=FALSE,fig.width=8 , fig.height=6}
...
ggplot(agg.data4B, aes(x=as.factor(portes), y=trial   , fill=Injection))+
    geom_boxplot()+
  geom_jitter(aes(colour=Injection), position = position_jitterdodge(), size=2)+
...
hom.chtype2.glm <- glm(trial ~ choice_type /Injection  , family = "poisson", data=subset(agg.data4B, singe=="Homer" & portes=="blue"))
...
Don.chtype2.glm <- glm(trial ~  choice_type/Injection  , family = "poisson", data=subset(agg.data4B, singe=="Donut" & portes=="blue"))
...
xyplot_data <- cbind(subset(agg.data4B,choice_type=="Correct" & portes =="blue"), subset(agg.data4B, choice_type=="Repeat"& portes =="blue"))
...
xyplot_data.first30 <- cbind(subset(agg.data4B.first30,choice_type=="Correct" & portes =="blue"), subset(agg.data4B.first30, choice_type=="Repeat"& portes =="blue"))

names(xyplot_data.first30) <- c("Injection","Correct", "session","portes","singe","Correct_trials",
                        "Injection1","Repeat", "session1","portes1","singe1","Repeat_trials")

ggplot(xyplot_data.first30, aes(x=Correct_trials, y=Repeat_trials, fill=Injection))+
  geom_point(aes(colour=Injection), size=4)+
...
#--------stats all trials------------
hom.chtypeCor.glm <- glm(trial ~ Injection * portes, family = "poisson", data=subset(agg.data4B, singe=="Homer" & choice_type == "Correct" ))
summary(hom.chtypeCor.glm)

Don.chtypeCor.glm <- glm(trial ~ Injection * portes , family = "poisson", data=subset(agg.data4B, singe=="Donut" & choice_type == "Correct"))
summary(Don.chtypeCor.glm)

hom.chtypeCor.glm <- glm(trial ~ Injection , family = "poisson", data=subset(agg.data4B, singe=="Homer" & choice_type == "Repeat" & portes =="blue"))
summary(hom.chtypeCor.glm)

Don.chtypeCor.glm <- glm(trial ~ Injection  , family = "poisson", data=subset(agg.data4B, singe=="Donut" & choice_type == "Repeat"& portes =="blue"))
summary(Don.chtypeCor.glm)
...
ggplot(subset(mean.nextdist, portes=="blue"), aes(choice_type, y=nextdist, fill=Injection))+
  geom_boxplot(outlier.shape = NA)+
  geom_point(aes(colour=Injection), position = position_jitterdodge())+
  facet_grid(singe~portes*Injection)+
  ylim(5,18)+
...
ggplot(mean.nextdist, aes(Injection, y=nextdist, fill=Injection))+
  geom_boxplot(outlier.shape = NA)+
  geom_point(aes(colour=Injection), position = position_jitterdodge())+
  facet_grid(singe~portes*choice_type)+
  ylim(5,18)+
...
lm.nextdist <- lm(nextdist ~ portes * choice_type * Injection, mean.nextdist)
summary(lm.nextdist)


lm.nextdist.Do <- lm(nextdist ~ Injection * portes * choice_type , subset(mean.nextdist, singe == "Donut"))
summary(lm.nextdist.Do)


lm.nextdist.Ho <- lm(nextdist ~ Injection * portes * choice_type , subset(mean.nextdist, singe == "Homer"))
summary(lm.nextdist.Ho)



lm.nextdist.Do <- lm(nextdist ~ Injection *  choice_type , subset(mean.nextdist, singe == "Donut" & portes =="blue"))
summary(lm.nextdist.Do)


lm.nextdist.Ho <- lm(nextdist ~ Injection  * choice_type , subset(mean.nextdist, singe == "Homer" & portes =="blue"))
summary(lm.nextdist.Ho)
...
ggplot(stats.repeat, aes(x=Injection, y=d2rpt,color=Injection))+
  geom_boxplot(aes(fill=Injection),size=1, alpha=.6)+
  facet_grid(portes~singe)+
     scale_color_manual(values=c("grey40","chartreuse3","dodgerblue1"))+
     scale_fill_manual(values=c("grey40","chartreuse3","dodgerblue1"))+
    ggtitle('distance between a choice and its repetition')

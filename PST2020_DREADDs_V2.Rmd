...
c <- ggplot(shortp, aes(x=Injection, y=choice, fill=Injection))+
   ...
   geom_jitter(aes(colour=Injection), width=0.1)+
    ...
    scale_fill_manual(values=c("chartreuse3","grey"))+
     scale_colour_manual(values=c("chartreuse4","grey40"))+
    ggtitle("Mean number of SHORT pauses during sessions (cod 99)")+ n
  theme(axis.text.x = element_text(color = "grey20", size = 12))+
  ylim(0,8)
...
hom.chtypeM.glm <- glm(trial ~ Injection * portes, family = "poisson", data=subset(agg.data4B.first30, singe=="Homer" & choice_type == "Miss" ))
summary(hom.chtypeM.glm)

Don.chtypeM.glm <- glm(trial ~ Injection * portes , family = "poisson", data=subset(agg.data4B.first30, singe=="Donut" & choice_type == "Miss"))
summary(Don.chtypeM.glm)

hom.chtypeRep.glm <- glm(trial ~ Injection * portes, family = "poisson", data=subset(agg.data4B.first30, singe=="Homer" & choice_type == "Correct" ))
summary(hom.chtypeRep.glm)

Don.chtypeRep.glm <- glm(trial ~ Injection * portes , family = "poisson", data=subset(agg.data4B.first30, singe=="Donut" & choice_type == "Correct"))
summary(Don.chtypeRep.glm)

hom.chtypeRep.glm <- glm(trial ~ Injection * portes, family = "poisson", data=subset(agg.data4B.first30, singe=="Homer" & choice_type == "Repeat" ))
summary(hom.chtypeRep.glm)

Don.chtypeRep.glm <- glm(trial ~ Injection * portes , family = "poisson", data=subset(agg.data4B.first30, singe=="Donut" & choice_type == "Repeat"))
summary(Don.chtypeRep.glm)
  
hom.chtype.glm <- glm(trial ~ Injection * portes * choice_type, family = "poisson", data=subset(agg.data4B.first30, singe=="Homer" ))
summary(hom.chtype.glm)
  
Don.chtype.glm <- glm(trial ~ Injection * portes* choice_type, family = "poisson", data=subset(agg.data4B.first30, singe=="Homer" ))
summary(Don.chtype.glm)
